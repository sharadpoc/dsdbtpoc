...
SELECT
        ...
        FROM {{ ref('mov_srt_ewm_fcy_ar') }} AS mov_SRT_EWM_FCY_AR
          INNER JOIN
            {{ ref('mov_srt_ewm_ar_x_cl_rtg_r') }} AS mov_SRT_EWM_AR_X_CL_RTG_R
          ON mov_SRT_EWM_FCY_AR.SRC_DL = mov_SRT_EWM_AR_X_CL_RTG_R.SRC_DL AND mov_SRT_EWM_FCY_AR.IP_ID = mov_SRT_EWM_AR_X_CL_RTG_R.IP_ID
)
